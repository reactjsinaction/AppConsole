...
class TypeHandlerTests: XCTestCase {
    ...
    func test_getter_handle() {
        ...
        let typ = (val as! ValueObject).type
        Assert.True(["{CGRect={CGPoint=dd}{CGSize=dd}}", "{CGRect={CGPoint=ff}{CGSize=ff}}"].contains(typ))
    ...
    func test_UIColorFromString() {
        if let color = UIColorFromString("UIDeviceRGBColorSpace 0 1 1 1") {
            Assert.equal("UIDeviceRGBColorSpace 0 1 1 1", String(color))
        }

        if let white = UIColorFromString("UIDeviceWhiteColorSpace 0 1") {
